...
namespace FormularzWyjazdu
{
    public partial class Form1 : Form
    {
        ...
        {
            if(LeaveDate.Value < ArriveDate.Value || ArriveDate.Value < ActualDate.Value)
            {
                MessageBox.Show("Ustaw poprawną datę odjazdu!");
            }
            else
            {
                NumOfDays.Text = (LeaveDate.Value.Date - ArriveDate.Value.Date).Days.ToString() + " (" + (ArriveDate.Value.Date - ActualDate.Value.Date).Days.ToString() + " dni do wyjazdu)";
            }
        }
